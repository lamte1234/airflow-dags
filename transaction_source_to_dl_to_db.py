...
with DAG("transaction_source_to_dl_to_db", schedule_interval="@once", default_args=default_args, catchup=False) as dag:
	task_1 = SparkSubmitOperator(
		task_id = "task_1",
		application= "/mnt/c/airflow/jars/mockproject_2.12-0.1.jar",
		conn_id = "spark_conn_local",
		java_class="markettingtask.TransactionsSyncToDL",
	...
	task_2 = SparkSubmitOperator(
		...
		application= "/mnt/c/airflow/jars/mockproject_2.12-0.1.jar",
		conn_id = "spark_conn_local",
		java_class="markettingtask.ActivityTableTask",
		# application_args = [str({{ macros.ds_add(ds, -1) }})] ## or some random day watev
	)
